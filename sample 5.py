...
def main():
    ...
    # Fit hyperparameter tuning on original train data
    ...
    # Transform training data manually for resampling
    ...
    X_over, y_over = oversampler.fit_resample(X_train_transformed, y_train)
    print("After Random Oversampling:", np.bincount(y_over))

    undersampler = RandomUnderSampler(random_state=42)
    X_resampled, y_resampled = undersampler.fit_resample(X_over, y_over)
    print("After Random Undersampling:", np.bincount(y_resampled))

    # Retrain classifier on resampled data
